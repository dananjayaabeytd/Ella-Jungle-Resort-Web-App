...
export default function CustomPackage() {
  ...
  const handleCreatePackage = async () => {
    try {
      const packageData = {
        user_id: '660793227a87f77d585cca4b',
      ...
      const response = await axios.post('http://localhost:8081/custom_packages/add', packageData);
  ...
  return (
    <>
      <div className='relative mx-[550px] my-[40px]'>
        <Button className='mx-[5px]' onClick={() => setActiveSection('rooms')}>
          Rooms
        </Button>
        <Button className='mx-[5px]' onClick={() => setActiveSection('activities')}>
          Special Activities
        </Button>
        <Button className='mx-[5px]' onClick={() => setActiveSection('spas')}>
          Spa
        </Button>
      </div>
            ...
            {selectedItems.map((item, index) => (
              <tr key={index} className='border border-gray-400'>
                <td className='px-4 py-2 border border-gray-400'>
                  {item.name || item.roomName || item.spaPackageName}
                </td>
                <td className='px-4 py-2 border border-gray-400'>
                  Rs.{item.price}
                </td>
