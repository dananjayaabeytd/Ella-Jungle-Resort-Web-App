...
const HotelBookingForm = () => {
  ...
  const validateCreditCard = value => {
    // Check if value contains only numerical digits
    return /^\d+$/.test(value);
  };

  const validateNameOnCard = value => {
    // Check if value contains only letters (alphabets)
  ...
  const handleValidation = () => {
    const errors = {};
    if (!formData.checkin_date || !formData.checkout_date) {
      errors.checkin_date = 'Check-in and check-out dates are required.';
    } else if (new Date(formData.checkout_date) < new Date(formData.checkin_date)) {
      errors.checkout_date = 'Check-out date cannot be before check-in date.';
    }

    if (!validateCreditCard(formData.credit_card_no)) {
      errors.credit_card_no = 'Credit card number must be numerical.';
    }

    if (!validateNameOnCard(formData.name_on_card)) {
      errors.name_on_card = 'Name on card can only contain letters.';
    }
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <input
            ...
            required
          />
          {errors.checkin_date && <p className='text-red-500'>{errors.checkin_date}</p>}
        </div>
        <div>
          ...
          <input
            ...
            required
          />
          {errors.checkout_date && <p className='text-red-500'>{errors.checkout_date}</p>}
        </div>
        <div>
          ...
          <input
            ...
            required
          />
          {errors.credit_card_no && <p className='text-red-500'>{errors.credit_card_no}</p>}
        </div>
        <div>
          ...
          <input
            ...
            required
          />
          {errors.name_on_card && <p className='text-red-500'>{errors.name_on_card}</p>}
        </div>
        <div>
