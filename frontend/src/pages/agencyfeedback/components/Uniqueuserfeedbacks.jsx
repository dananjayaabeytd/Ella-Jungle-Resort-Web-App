...
import Deleteagencyfeedback from "./Deleteagencyfeedback";
import Updateagencyfeedback from "./Updateagencyfeedback";
...
const Uniqueuserfeedback = () => {
  ...
  const fetchFeedbacks = async (giverId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/agencyfeedbacks/feedbacksByGiverId/${giverId}`);
  ...
  const handleDeleteFeedback = async (feedbackId) => {
    try {
      await axios.delete(`http://localhost:5000/api/agencyfeedbacks/deletefeedback/${feedbackId}`);
              ...
              <div className="flex justify-end space-x-2">
                {showUpdateForm[feedback._id] ? (
                  ...
                  <>
                    ...
                    <Deleteagencyfeedback feedbackId={feedback._id} onDeleteFeedback={() => handleDeleteFeedback(feedback._id)} />
                  </>
                )}
              </div>
            </li>
...
export default Uniqueuserfeedback;
